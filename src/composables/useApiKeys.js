...
export function useApiKeys() {
  ...
  const validateClaudeApiKey = (key) => {
    ...
    if (!key.startsWith('sk-ant-')) return 'Claude API key should start with "sk-ant-"'
    return null
  }

  // Computed validation states
  const isLifxKeyValid = computed(() => validateLifxApiKey(lifxApiKey.value) === null)
  const isClaudeKeyValid = computed(() => validateClaudeApiKey(claudeApiKey.value) === null)
  const areKeysValid = computed(() => isLifxKeyValid.value && isClaudeKeyValid.value)
