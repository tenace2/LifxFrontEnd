<template>
  <q-card class="chat-card">
    ...
    <q-card-section class="chat-container">
      ...
      >
        <div v-if="messages.length === 0" class="text-center q-pa-lg text-grey-6">
        ...
        <div v-for="(message, index) in messages" :key="index" class="message-container">
          ...
          <div v-else class="claude-message">
            ...
            <!-- Action Indicators -->
            <div v-if="message.actions && message.actions.length > 0" class="q-mt-sm q-ml-md">
              <q-chip 
...
const $q = useQuasar()
const { claudeApiKey, lifxApiKey, isClaudeKeyValid, isLifxKeyValid } = useApiKeys()
...
const lightKeywords = ['light', 'lamp', 'color', 'bright', 'dim', 'on', 'off', 'lifx', 'hue', 'saturation', 'warm', 'cold', 'red', 'blue', 'green', 'yellow', 'purple', 'orange', 'white']

// Computed properties
const canSendMessage = computed(() => {
  return isClaudeKeyValid.value && 
...
const sendMessage = async () => {
  ...
  if (!hasLightKeywords.value) {
    const errorMessage = {
      role: 'error',
      content: 'Please ask questions related to LIFX light control. For example: "Turn on the living room lights" or "Set all lights to blue".',
      timestamp: Date.now()
    }
