			...
			<div v-if="requestsRemaining <= 10" class="q-mt-sm">
				...
				>
					...
					<span v-if="requestsRemaining === 0">
						Request limit reached. Please try again later or start a new
						session.
					</span>
					<span v-else>
			...
			<q-card style="min-width: 300px">
				...
				<q-card-section class="q-pt-none">
						...
						<q-item>
							<q-item-section>
								...
								<q-item-label caption
									>{{ requestsUsed }} / {{ dailyLimit }}</q-item-label
								>
							</q-item-section>
						</q-item>
					...
					<div class="q-mt-md text-caption text-grey-7">
						<strong>Note:</strong> This is a demo application with usage limits
						to prevent abuse. Sessions are tracked by IP address and browser
						session.
					</div>
				</q-card-section>
...
<script setup>
	...
	const confirmResetSession = () => {
		$q.dialog({
			title: 'Reset Session',
			message:
				'Are you sure you want to reset your session? This will clear your usage tracking.',
			cancel: true,
			persistent: true,
		}).onOk(() => {
			resetSession();
