    ...
    <q-card-section>
      ...
      <div v-if="isLifxKeyValid && backendStatus === 'connected'">
        ...
        <div v-if="lights.length > 0" class="q-mb-md">
          <div class="text-subtitle2 q-mb-sm">Your LIFX Lights ({{ lights.length }})</div>
          <q-list bordered separator>
            <q-item v-for="light in lights" :key="light.id" clickable @click="selectLight(light)">
              <q-item-section avatar>
                <q-avatar :color="light.connected ? 'positive' : 'grey'" icon="lightbulb" />
              </q-item-section>
              <q-item-section>
        ...
        <div v-if="selectedLight" class="q-mb-md">
          ...
          <div class="row q-gutter-md q-mb-md">
            <q-btn 
              ...
              :icon="selectedLight.power === 'on' ? 'lightbulb' : 'lightbulb_outline'"
          ...
          <!-- Brightness Control -->
          <div class="q-mb-md">
            <div class="text-caption q-mb-xs">Brightness: {{ Math.round(brightness * 100) }}%</div>
            <q-slider
              v-model="brightness"
              :min="0.01"
              :max="1"
              :step="0.01"
              :disable="selectedLight.power === 'off' || !selectedLight.connected"
          ...
          <div class="q-mb-md">
            ...
            <div class="row q-gutter-sm q-mb-sm">
              <q-btn 
                ...
                :disable="selectedLight.power === 'off' || !selectedLight.connected || !canSendRequest()"
                class="col-auto"
              />
            </div>
      ...
      <div v-else class="text-center q-pa-md">
        ...
        <div class="text-grey-7">
          <div v-if="!isLifxKeyValid">Please enter your LIFX API key above to control lights</div>
          <div v-else-if="backendStatus !== 'connected'">Backend server connection required</div>
        </div>
      </div>
    </q-card-section>
  </q-card>
...
const allLightsAction = async (action) => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      throw new Error(response.data.error || `Failed to turn all lights ${action}`)
    }
  } catch (error) {
    $q.notify({
      type: 'negative',
